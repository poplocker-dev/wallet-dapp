...
function txHistory(state = null, action) {
  switch (action.type) {
    case 'ETH_TX_HISTORY_UPDATE':
      return action.transactions ? [...action.transactions] : null;
    default:
      return state;
  }
}
...
function txListAddress(state = null, action) {
  if (action.type == 'ETH_TX_LIST_ADDRESS_UPDATE') {
    return action;
  }
  else
    return state;
...
const reducers = combineReducers({ connection, address, balance, txHistory, pendingTxs, txListAddress, locker });
